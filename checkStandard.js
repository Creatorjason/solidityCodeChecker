...
function getStandard(args){
    ...
    window.localStorage.setItem("standard", `${standard_contract_types[`${args}`]}`)
    //window.localStorage.clearItem("value")
}
function getVal(){
    buttonCompare_Clicked()
    window.localStorage.clearItem("diff")
...
{      
    ...
    var textAfter = document.getElementById("cont").value;
    ...
    var textareaDifferences = document.getElementById("divComparison");
    textareaDifferences.innerHTML = differencesAsString;
    //window.localStorage.setItem("analyze",differencesAsString)
   
   
   
    console.log(differencesAsString)
      
}
function runAnalysis(){
    if(window.localStorage.getItem("spill") != null){
     console.log("loaded ready to go")
 }
 else{
    console.log("Uhn")
 }
...
}
{
    TextPassagePair.prototype.toString = function()
    {
        ...
        else
        {
            ...
            // document.getElementById("code-stop").innerHTML += returnValue
            // returnValue += this.escapeStringForHTML(this.passages[1].text);
            // document.getElementById("code-go").innerHTML += returnValue
            //returnValue += "</mark>";
            returnValue += "<mark style='background-color:yellow'>";
            returnValue += this.escapeStringForHTML(this.passages[0].text);
            
            returnValue += "</mark><mark style='background-color:rgb(62, 200, 62)'>";
            returnValue += this.escapeStringForHTML(this.passages[1].text);
